...
impl GameboardView {
        ...
        C: CharacterCache<Texture = G::Texture>,
    {
        ...
        // Generate and draw lines for the Sudoku Grid
        for i in 0..9 {
            let x1 = settings.position[0] + i as f64 / 9.0 * settings.size;
            let y1 = settings.position[1] + i as f64 / 9.0 * settings.size;
            let x2 = settings.position[0] + settings.size;
            let y2 = settings.position[1] + settings.size;

            let vline = [x1, settings.position[1], x1, y2];
            let hline = [settings.position[0], y1, x2, y1];
            ...
            // Otherwise draw regular cell lines
            else {
